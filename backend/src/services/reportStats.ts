...
import type { ThreatEvent, SensorReading } from '../types/database';
...
export async function computeMonthlyStats(
  ...
  const { data: threatsRaw, error: tErr } = await supabaseAdmin
    .from('threat_events')
    .select('*')
    ...
    .lt('occurred_at', to);
  if (tErr) throw tErr;
  const threats = (threatsRaw ?? []) as ThreatEvent[];

  const { data: readingsRaw, error: rErr } = await supabaseAdmin
    ...
    .lt('recorded_at', to);
  if (rErr) throw rErr;
  const readings = (readingsRaw ?? []) as SensorReading[];

  const surges = threats.filter((t) => t.event_type === 'surge').length;
  const intrusions = threats.filter(
    (t) => t.event_type === 'intrusion'
  ).length;
  ...
  const alertaResolved = threats.filter(
    (t) => t.alerta_status === 'ack' || t.alerta_status === 'closed'
  ).length;
